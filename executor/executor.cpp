 ...
 * 私有函数定义区
 *------------------------------------------------------------------*/
Selectnode* grammar2plan(trivialtree * root);
void* exec_pretreatment();
...
void execute_sql(char *sql, int strlen){
    ...
    printf("grammar2plan\n");
    // 生成查询计划
    Selectnode *f = grammar2plan(root);

    printf("exec prepare ready.\n");

    Plannode *logicplan = plan_init(f);

    printf("logic plan ready.\n");
}
